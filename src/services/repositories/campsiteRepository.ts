...
const COUNTRIES: CountryRegion[] = [
  {
    name: "Norway",
    regions: ["Lofoten Islands", "Nordkapp", "Tromsø", "Bergen", "Stavanger"],
    coords: [10.0, 60.0],
  },
  {
    name: "Sweden",
    regions: ["Lapland", "Gothenburg", "Stockholm", "Malmö", "Kiruna"],
    coords: [15.0, 62.0],
  },
  {
    name: "Finland",
    regions: ["Lapland", "Helsinki", "Turku", "Rovaniemi", "Oulu"],
    coords: [25.0, 64.0],
  },
  {
    name: "Iceland",
    regions: ["Reykjavik", "Akureyri", "Westfjords", "South Coast", "Highlands"],
    coords: [-19.0, 64.0],
  },
  {
    name: "Switzerland",
    regions: ["Bernese Oberland", "Valais", "Graubünden", "Ticino", "Central Switzerland"],
    coords: [8.0, 47.0],
  },
  {
    name: "Austria",
    regions: ["Tyrol", "Salzburg", "Carinthia", "Styria", "Upper Austria"],
    coords: [14.0, 47.5],
  },
  {
    name: "Germany",
    regions: ["Black Forest", "Bavarian Alps", "Saxon Switzerland", "Harz Mountains", "Eifel"],
    coords: [10.0, 51.0],
  },
  {
    name: "France",
    regions: ["French Alps", "Pyrenees", "Provence", "Brittany", "Corsica"],
    coords: [2.0, 46.0],
  },
  {
    name: "Italy",
    regions: ["Tuscany", "Dolomites", "Lake Como", "Amalfi Coast", "Sicily"],
    coords: [12.0, 42.0],
  },
  {
    name: "Portugal",
    regions: ["Algarve Coast", "Porto", "Azores", "Madeira", "Central Portugal"],
    coords: [-8.0, 39.5],
  },
  {
    name: "Spain",
    regions: ["Pyrenees", "Andalusia", "Galicia", "Basque Country", "Canary Islands"],
    coords: [-4.0, 40.0],
  },
  {
    name: "Croatia",
    regions: ["Dalmatia", "Istria", "Plitvice", "Krka", "Kornati"],
    coords: [16.0, 45.0],
  },
  {
    name: "Slovenia",
    regions: ["Julian Alps", "Triglav", "Lake Bled", "Postojna", "Maribor"],
    coords: [14.5, 46.0],
  },
  {
    name: "Czech Republic",
    regions: ["Bohemian Paradise", "Krkonoše", "Šumava", "Moravian Karst", "Jeseníky"],
    coords: [15.5, 49.5],
  },
  {
    name: "Poland",
    regions: ["Tatra Mountains", "Zakopane", "Białowieża", "Mazury", "Sudetes"],
    coords: [19.0, 52.0],
  },
];
...
const CAMPSITE_NAMES = [
  "Aurora Valley Wilderness",
  "Alpine Serenity Base",
  "Midnight Sun Fjord",
  "Black Forest Sanctuary",
  "Coastal Dunes Paradise",
  "Tuscany Hills Retreat",
  "Nordic Lights Haven",
  "Mountain Peak Lodge",
  "Forest Whisper Camp",
  "Ocean Breeze Sanctuary",
  "Starlight Meadows",
  ...
  "Golden Hour Plateau",
  "Misty Mountain Base",
  "Emerald Forest Lodge",
  "Silver Stream Camp",
  "Thunder Valley Retreat",
  "Peaceful Harbor",
  "Adventure Peak Base",
  "Serenity Springs",
  "Wilderness Edge Camp",
  "Nature's Gateway",
  "Harmony Hills",
  "Tranquil Waters",
  "Explorer's Paradise",
...
const seededNumber = (seed: number, min: number, max: number): number =>
  Math.floor(fractional(seed) * (max - min + 1)) + min;
...
export class CampsiteRepository {
  ...
  private matchesFilters(campsite: Campsite, filters: CampsiteSearchFilters): boolean {
    ...
    if (
      filters.country &&
      campsite.location.country.toLowerCase() !== filters.country.toLowerCase()
    ) {
      return false;
    }
  ...
  private createCampsite(index: number): Campsite {
    ...
    const basePrice =
      country.name === "Switzerland" || country.name === "Norway"
        ? 60
        : country.name === "Germany" || country.name === "Austria"
          ? 45
          : country.name === "Portugal" || country.name === "Poland"
            ? 25
            : 35;
    ...
    const campsite: Campsite = {
      ...
      price_per_night: Math.round(
        basePrice * difficultyMultiplier + seededNumber(index + 31, -10, 10),
      ),
      images: imagePool,
      weather: {
  ...
  private createReviews(campsiteId: string, index: number): Review[] {
    ...
    return Array.from({ length: count }, (_, reviewIndex) => ({
      ...
      date: new Date(
        Date.now() - seededNumber(index + reviewIndex + 67, 1, 180) * 24 * 60 * 60 * 1000,
      )
        .toISOString()
        .split("T")[0],
    }));
